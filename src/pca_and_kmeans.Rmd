...
X = final_df.drop(['Hospital overall rating'], axis=1)
y = final_df[['Hospital overall rating']]
...
kmeans_model = KMeans(n_clusters = 4, max_iter=100,random_state = 100)
kmeans_model.fit(X)

labelled_df = X.copy()

labelled_df['Cluster ID'] =  kmeans_model.labels_
...
```
```{python}
ax = sns.boxplot(x="radm_score", y="Cluster ID", data=labelled_df)
plt.plot()
```

```{python}
cleaned_master_data_numerical_values['Cluster ID'].value_counts()
